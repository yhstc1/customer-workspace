...
function editCheckinMobile(cid) {
    ...
    .catch(function(e) { showToast('更新失败: ' + (e && e.message ? e.message : e), 'error'); });
}
...
async function changePasswordMobile() {
    ...
    try {
        ...
        const data = e && e.data;
        msgEl.innerHTML = '<span style="color:#c0392b;"><i class="m-icon" data-icon="error"></i> ' + ((data && data.error) || '修改失败') + '</span>';
    }
}
