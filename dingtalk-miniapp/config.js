// 后端 API 基础地址：部署到阿里云 FC 后填入其 HTTP 触发器公网端点
// 本地/联调阶段可先填 ngrok 或 FC 临时地址
const config = {
  // 阿里云 FC HTTP 触发器公网端点
  apiBase: 'https://crm-api-fc-nrfocbaaxv.cn-hangzhou.fcapp.run',
  // Workbuddy连接 组织 corpId（免登 requestAuthCode 必填）
  corpId: 'ding49b7555f7b0e7c421b9a8c00fa015bc5',
  // 本应用 AppKey（真小程序 2026-08-23 重建，AgentId 4888300256，MiniAppId 5000000006851992）
  appKey: 'ding7bsv8daxdq1cr1jb',
  // 开发调试：模拟器拿不到 authCode 时允许手动输入（正式上线前关闭）
  enableManualAuthCode: true
};

module.exports = config;
