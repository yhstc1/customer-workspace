...
本指南帮助你在自己的电脑（Windows / macOS / Linux）上运行客户管理工作空间。
...
依赖只有三个：`flask`、`requests`、`dingtalk-sdk`（体积很小，几秒钟装完）。
...
============================================================
  ...
  钉钉入口:    http://localhost:5000/dingtalk
============================================================
```
...
> 见 `deploy.sh`（内网穿透 / 云服务器）和 `钉钉集成指南.md`。
...
详见 `钉钉集成指南.md`。核心步骤：
1. 钉钉开放平台创建 H5 微应用，拿到 AppKey/AppSecret/AgentId/CorpId
2. 用环境变量或 `dingtalk_config.py` 配置
3. 钉钉应用首页地址填 `https://你的公网域名/dingtalk`
4. 在「设置」页点「测试钉钉连接」验证
